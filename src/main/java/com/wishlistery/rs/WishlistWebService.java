...
public class WishlistWebService {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Wishlist updateWishlistById(@PathParam("id") String id, Wishlist updated) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Wishlist quickEditWishlistById(@PathParam("id") String id, String text, @QueryParam("preview") boolean preview) {
        Wishlist wishlist = findWishlist(id);
        wishlist.quickEdit(text);
        if (!preview) {
            wishlistRepo.save(wishlist);
        }
        return wishlist;
